...
inline int get_attr(uint32_t rayID, uint32_t attrID) {
    int ret;
...
static __attribute__((always_inline)) void commit(uint32_t rayID) {    
    register uint32_t rs_rayID __asm__("x1") = rayID;

    __asm__ volatile (".insn r %[insn], 3, 3, x0, x%[rs_id], x0" :: [insn] "i" (RISCV_CUSTOM0), [rs_id]"i"(action), "r"(rs_rayID));
}

// IS accept
static __attribute__((always_inline)) void commit(uint32_t rayID, float t, float u, float v, uint32_t primitiveID) {
    register float rs_t __asm__("f6") = t;
    register float rs_u __asm__("f7") = u;
    register float rs_v __asm__("f8") = v;
    register uint32_t rs_pid __asm__("x9") = primitiveID;

    __asm__ volatile (
        ".insn r %[insn], 4, 3, x0, %[rs_id], %[rs_hit]"
        :: [insn] "i"(RISCV_CUSTOM0)
        , [rs_id] "r"(rayID)
        , [rs_hit] "f"(rs_t), "f"(rs_u), "f"(rs_v), "r"(rs_pid)
    );
}
